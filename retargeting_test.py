...
SIDE_LEN   = 0.10           # square side length [m]
PERIM      = 4.0 * SIDE_LEN
RADIUS     = SIDE_LEN / 2.0 # circle radius     [m]

SPEED      = 0.1            # linear [m/s]  (square, circle)
ANGLE_DEG  = 30.0           # orientation amplitude
ORI_PERIOD = 4.0            # orientation period [s]
...
# ===================================================================
class RightHandPub:
    """Publishes PoseArray for pelvis, chest, right-hand, HMD."""
    def __init__(self, mode: str):
        self.mode = mode.lower()           # e.g. square_xy, circle_xz …
        self.enabled = False
        ...
        self.static_poses = []
        for pos, ori in zip(READY_POS, READY_ORI):
            p = Pose()
        ...
        self.ready_pose = PoseArray()
        self.ready_pose.poses.extend(self.static_poses)

    # --------------------------------------------------------------
    def guilog_cb(self, msg):
        if TRIGGER_TEXT in msg.data and not self.enabled:
            rospy.loginfo("Trigger received → trajectory starts")
            self.enabled    = True
            self.start_time = rospy.Time.now()
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            if self.enabled:
                ...
                self.ready_pose.header = Header(stamp=rospy.Time.now(),
                                                frame_id=FRAME_ID)
                self.pub.publish(self.ready_pose)
                self.status_p.publish(Bool(data=True))
            rate.sleep()
    ...
    def publish_tracker_pose(self):
        ...
        is_square = self.mode.startswith("square_")
        is_circle = self.mode.startswith("circle_")
        is_ori    = self.mode in ("roll", "pitch", "yaw")

        if is_square or is_circle:
            plane = self.mode.split('_')[-1]   # xy/xz/yz
            if is_square:
                s = (t * SPEED) % PERIM
                if   s < SIDE_LEN:           u, v = 0.0, s
                elif s < 2*SIDE_LEN:         u, v = s - SIDE_LEN, SIDE_LEN
                ...
                theta = (t * SPEED / RADIUS) % (2.0 * math.pi)
                u, v  = RADIUS * math.cos(theta), RADIUS * math.sin(theta)

            if   plane == "xy":  x += u; y += v
            elif plane == "xz":  x += u; z += v
            else:                y += u; z += v

        elif is_ori:
            amp   = math.radians(ANGLE_DEG)
            angle = amp * math.sin(2.0 * math.pi * t / ORI_PERIOD)
            axis  = {"roll":(1,0,0),"pitch":(0,1,0),"yaw":(0,0,1)}[self.mode]
            q = quaternion_multiply(self.rh_q,
        ...
        pa = PoseArray()
        pa.header = Header(stamp=now, frame_id=FRAME_ID)
        pa.poses.extend(self.static_poses[0:5])  # pelvis~R-upperarm
        pa.poses.append(rh)                      # R-hand (moving)
        pa.poses.extend(self.static_poses[6:7])  # HMD
...
# ===================================================================
def main():
    parser = argparse.ArgumentParser(description="Right-hand motion publisher")
    parser.add_argument(
        "--mode",
        choices=[
            "square_xy", "square_xz", "square_yz",
            "circle_xy", "circle_xz", "circle_yz",
            "roll", "pitch", "yaw"
        ],
        default="square_xy",
        help="trajectory mode (default: square_xy)"
    )
    args = parser.parse_args()

    rospy.init_node("right_hand_motion_pub", anonymous=False)
    RightHandPub(args.mode).spin()
